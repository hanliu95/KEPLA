...
class KEPLA(nn.Module):
    def __init__(self, **config):
        ...
        mlp_in_dim = 512 # config["DECODER"]["IN_DIM"]
    ...
    def forward(self, bg_d, v_p, smiles, ids, mode="train"):
        ...
        attn_d = torch.matmul(
            v_d,
            v_p_global.unsqueeze(2)
        ).transpose(1, 2)

        attn_d = attn_d / math.sqrt(v_d.size(-1))
        ...
        attn_p = torch.matmul(
            v_p,
            v_d_global.unsqueeze(2)
        ).transpose(1, 2)

        attn_p = attn_p / math.sqrt(v_p.size(-1))
...
class KGEModel(nn.Module):
    def __init__(self, nentity, nrelation, entity_dim, gamma):
        ...
        self.KG_dict = np.load(
            "./datasets/KG_GO_dict.npy",
            allow_pickle=True
        ).item()

        self.mol_KG_dict = np.load(
            "./datasets/KG_MD_dict.npy",
            allow_pickle=True
        ).item()

        self.mol_feat_KG_dict = np.load(
            "./datasets/KG_CF_dict.npy",
            allow_pickle=True
        ).item()
    ...
    def forward(self, v_d, v_p, smiles, ids):
        ...
        for count, id in enumerate(ids):
            if id in self.KG_dict:
                r_t = np.array(self.KG_dict[id])
        ...
        if len(h_index) > 0:
            h_index = torch.LongTensor(h_index).to(device)
            r_index = torch.LongTensor(list(map(int, r_index))).to(device)
            t_index = torch.LongTensor(list(map(int, t_index))).to(device)
        ...
        # =========================
        # Molecule feature KG: KG_CF_dict
        # =========================
        mol_feat_h_index = []
        mol_feat_r_index = []
        mol_feat_t_index = []
        ...
        for count, smi in enumerate(smiles):
            if smi in self.mol_KG_dict:
                r_t = np.array(self.mol_KG_dict[smi])
            ...
            if smi in self.mol_feat_KG_dict:
                feat_rt = np.array(self.mol_feat_KG_dict[smi])
        ...
        # if missing_mol > 0 or missing_mol_feat > 0:
        #     print(
        #         f"[Warning] missing mol_KG_dict: {missing_mol}, "
        #         f"missing mol_feat_KG_dict: {missing_mol_feat}"
        #     )

        if len(mol_h_index) > 0:
            mol_h_index = torch.LongTensor(mol_h_index).to(device)
            mol_r_index = torch.LongTensor(list(map(int, mol_r_index))).to(device)
            mol_t_index = torch.LongTensor(list(map(int, mol_t_index))).to(device)
            ...
            kg_scores.append(mol_kg_score * 0.1)

        if len(mol_feat_h_index) > 0:
            mol_feat_h_index = torch.LongTensor(mol_feat_h_index).to(device)
            mol_feat_r_index = torch.LongTensor(
                list(map(int, mol_feat_r_index))
            ).to(device)
            mol_feat_t_index = torch.LongTensor(
                list(map(int, mol_feat_t_index))
            ).to(device)

            mol_feat_head = torch.index_select(
                v_d,
                0,
                mol_feat_h_index
            ).unsqueeze(1)

            mol_feat_relation = self.mol_feat_relation_embedding(
                mol_feat_r_index
            ).unsqueeze(1)

            mol_feat_tail = self.mol_feat_entity_embedding(
                mol_feat_t_index
            ).unsqueeze(1)

            mol_feat_kg_score = self.TransE(
                mol_feat_head,
                mol_feat_relation,
                mol_feat_tail,
                mode="head-batch"
            ...
            kg_scores.append(mol_feat_kg_score * 0.1)
        ...
        kg_score = torch.cat(kg_scores, dim=0)
